...
POLL_INTERVAL = 5
POLL_MAX      = 120  # 最多轮询 120 次（约 10 分钟）

EMOTION_STYLE = {
    "悲壮": "low saturation, cold gray tones, backlit silhouette, wide angle overhead shot, thick fog or overcast sky",
    "紧张": "high contrast hard light, close-up or tilted composition, dark red or deep orange tones, oppressive atmosphere",
    "孤独": "large negative space, single figure placed at frame edge, cool blue-gray tones",
    "辉煌": "golden warm light, low angle looking up, sweeping wide shot, expanding light rays",
    "压抑": "shadows consuming most of the frame, low angle, enclosed space, dark green or deep brown",
    "释然": "soft diffused light, shallow depth of field bokeh, warm white or light gold, horizontal composition",
}
...
def chat(model: str, system: str, user: str) -> str:
    """调用 WeryAI Chat Completion，返回回复文本。"""
    payload = {
        "model": model,
        "messages": [
    ...
    resp = req_post("/chat/completions", payload, timeout=180)
...
7. 严禁输出天气预报、穿衣建议、带伞提醒等与老黄历无关的内容"""
        ...
        spielberg_prompt = f"""你是顶级纪录片解说词作家。
当前年份是 {current_year} 年，涉及时间跨度时请据此准确计算。
请为主题「{topic}」创作 {num_lines} 句纪录片旁白台词：
• 每句 {char_range} 字
{style_guide}
• 如果涉及老黄历、彭祖百忌、宜忌等传统禁忌内容，必须用现代白话做安全化解读，不能照搬古文原句。例如"未不服药毒气入肠"不能直译为"不宜吃药看病"，应解读为"今日不宜尝试新药方，日常用药照常"。避免误导中老年用户做出危害健康的决定
...
VISUAL_CONTINUITY: ★跨分镜一致性锚★（全片 N 张图必须保持同一画风、同一主角、同一场景的视觉连贯）。需要包含：①主角锚（Hero Anchor）：一位固定主角的极详细外观描述（年龄、性别、发型、五官特征、校服/服饰细节、身高比例），全片所有出现主角的画面都用完全一致的描述；②场景锚（Setting Anchor）：2-3 个核心场景的固定元素（如"food tray with cyan holographic menu"、"wall with 光盘行动 star badge board"）；③风格锚（Style Anchor）：一个 5-10 词的英文 short stylebook，每张图 prompt 都要原样引用（如："cheerful 3D cartoon, pixar-style soft lighting, warm yellow-blue palette"）；④中文元素点（Chinese Text Anchors，仅在主题涉及中国场所/机构时给出）：列出该主题必须在画面里自然出现的中文字符串（如校牌/门牌/徽章上的"XX 学校"），标明在哪几个分镜里出现更自然（如"开场 1-2 号、食堂入口 3-4 号、勋章 10-11 号"）
TABOOS: 画面禁区（例：{'儿童/校园题材禁 sepia / 老照片 / 历史服饰 / 阴郁低饱和 / 成人苦难叙事' if tone == '轻松' else '历史题材禁 cartoon / 明亮儿童插画 / anachronistic 现代设备'}；同时列出该主题特有的禁区）。注意：不要在 TABOOS 里禁止"文字/text"，本片支持 Nano Banana 2 渲染中文
...
• 开场引入：翻开的日历/年历，暖色光线
• 干支纳音：天干地支书法字符、五行（金木水火土）元素符号、中国传统纹样
• 十二建星：古代星盘、罗盘特写，配合吉凶暗示的光影
• 二十八星宿：夜空星图、古代天文仪器、星宿对应的动物形象
• 彭祖百忌：古代民间生活场景（厨房、药铺、织布）
• 宜忌：宜用暖色吉祥场景，忌用冷色警示场景
• 吉神凶神：中国传统神像、门神、符咒、庙宇
• 冲煞：十二生肖对冲图、方位避讳示意图、传统八卦方位盘
• 胎神占方：传统家居布局俯视图、胎神方位标注、古代内室场景
• 九宫飞星：九宫格方位图、洛书数字排列、风水罗盘上的飞星标注
• 五神方位：风水罗盘、方位图、财神爷/喜神/福神形象
• 吉时：日晷、更鼓、古代报时器、清晨/正午不同时辰的光线
• 收尾祝福：温馨家庭场景、红灯笼、福字

画面风格：中国传统美术风格，暖色调为主，工笔画/水墨画质感，高清细腻。
严禁使用写实人物照片风格，必须使用中国传统美术/插画风格。"""
    else:
        lines_display = chr(10).join(f'{i+1}. {l}' for i, l in enumerate(lines))
        almanac_visual_guide = ""

    jiangwen_prompt = f"""你是顶级电影导演姜文，擅长用强烈的视觉语言表达情绪。
...
• {ASPECT_RATIO} 英文画面提示词（70~100 词），要求：
...
只输出 JSON，不加任何说明文字。"""
    ...
    raw_json = chat("GEMINI_25_FLASH", "你是电影导演姜文，只输出 JSON。", jiangwen_prompt)
    ...
    for i, line in enumerate(lines):
        ...
        prompt  = visuals[i].get("prompt", "") + (f", {style}" if style else "")
...
def _send_for_approval(img_path: str, idx: int, text: str) -> None:
    """发送图片到 TG 带审批按钮，写 pending 信号文件。SSL 失败自动重试。"""
    ...
    for attempt in range(3):
        try:
            with open(img_path, "rb") as f:
                ...
                }, files={"photo": f}, timeout=(10, 30))
            return
        except Exception as e:
            log(f"审批图 {idx+1} 发送失败（第 {attempt+1} 次）：{e}")
            if attempt < 2:
                time.sleep(5)
...
def generate_bgm(topic: str, tone: str = "中性") -> str | None:
    ...
    MAX_BGM_RETRY = 3
    if tone == "轻松":
        bgm_desc = f"Cheerful upbeat children's background music for '{topic}', ukulele marimba glockenspiel whistle claps, warm playful hopeful mood, light and bouncy, family-friendly, no vocals"
    elif tone == "庄重":
        bgm_desc = f"Solemn reflective documentary soundtrack for '{topic}', slow strings and piano, restrained reverent atmosphere, no vocals"
    else:
        bgm_desc = f"Gentle contemplative Chinese documentary soundtrack for '{topic}', erhu guzheng pipa soft strings, calm thoughtful atmosphere, no vocals"
...
def step6_parallel(script: list[dict], topic: str) -> str | None:
    ...
    with ThreadPoolExecutor(max_workers=20) as ex:
        ...
        # 检查是否有图片缺失
        missing = [i+1 for i in range(n) if not completed.get(i)]
        if missing:
            raise RuntimeError(f"图片 {missing} 生成失败，无法继续")

        # ── 审批流程 ────────────────────────────────────────────────────
        if SKIP_APPROVAL:
            tg(f"⏭️ 跳过审批（--skip-approval），{n} 张图片直接通过")
            bgm_path = None
            try:
        ...
        tg(f"📋 {n} 张图片已生成，全部发送审批（可同时审核）...")
        for i, s in enumerate(script):
            _send_for_approval(s["img_path"], i, s["text"])
...
def _motion_one_scene(idx: int, scene: dict, motion_prompt: str, aspect_ratio: str) -> bool:
    ...
    try:
        ...
        img_url = ""
        data = up.get("data", {})
        url_list = data.get("object_url_list") or data.get("urls") or []
        if url_list and isinstance(url_list, list):
            img_url = url_list[0]
        if not img_url:
            img_url = data.get("url") or data.get("object_url") or ""
        if not img_url:
            log(f"[motion {idx}] 上传返回无 URL: {up}")
            return False

        # 2) 提交 image-to-video（WERYDANCE_2_0）
        r = req_post("/generation/image-to-video", {
            "model": "WERYDANCE_2_0",
            "image": img_url,
...
def step65_motion(script: list[dict]):
    """把静态 seg_N.mp4 替换为 WERYDANCE_2_0 动态版本（并发）"""
    n = len(script)
    ...
    # 2. 并发跑（WERYDANCE_2_0 aspect_ratio 必需；20 对齐 WeryAI 后台并发上限）
    aspect = "9:16" if IS_VERTICAL else "16:9"
    results = {}  # idx -> bool
    with ThreadPoolExecutor(max_workers=20) as ex:
        futs = {
            ex.submit(_motion_one_scene, i, scene, motion_prompts[i], aspect): i
        ...
        for fut in as_completed(futs):
            ...
            try:
                ...
                log(f"[motion {i}] future 异常: {e}")
                ok = False
            results[i] = ok
            if ok:
                tg(f"🎬 分镜 {i+1}/{n} 动态化 ✓")
            else:
                tg(f"⚠️ 分镜 {i+1}/{n} 动态化失败，保留静态版")

    success_cnt = sum(1 for v in results.values() if v)
    tg(f"✅ 动态化完成：{success_cnt}/{n} 成功 · {n - success_cnt} 保留静态兜底")
...
def step9_render(raw_path: str, voice_path: str, bgm_path: str | None, ass_path: str, topic: str) -> str:
    ...
    try:
        ...
        if voice_dur_chk > video_dur_chk + 1.0:
            ratio = voice_dur_chk / video_dur_chk
            log(f"音画同步修正：video {video_dur_chk:.2f}s → 配音 {voice_dur_chk:.2f}s（setpts ×{ratio:.3f}）")
            tg(f"🔧 音画同步修正：视频 {video_dur_chk:.1f}s → 拉伸到 {voice_dur_chk:.1f}s 匹配配音")
    ...
    if bgm_path:
        ffmpeg(
            ...
            "-filter_complex",
            "[1:a]volume=1.5[va];[2:a]volume=0.6[ba];[va][ba]amix=inputs=2:duration=first[aout]",
            "-map", "0:v",
            "-map", "[aout]",
            "-vf", f"scale={VIDEO_W}:{VIDEO_H},setsar=1,setdar={ASPECT_RATIO},ass={ass_escaped}",
            "-c:v", "libx264", "-crf", "20", "-preset", "medium",
            "-c:a", "aac", "-b:a", "128k",
            "-aspect", ASPECT_RATIO,
            "-movflags", "+faststart",
...
# ── 第十步：推送 Telegram ────────────────────────────────────────────────────
def _generate_caption(topic: str, script: list[dict]) -> tuple[str, str]:
    """用 LLM 生成社媒文案 + 短标题（按 tone 切爆款方法论）。返回 (文案, 短标题)。"""
    ...
    try:
        raw = chat("GEMINI_25_FLASH", "你是能出爆款的短视频文案策划师，熟悉视频号/抖音/小红书的点击钩子规律，会根据题材基调灵活切换受众和套路。",
            ...
            f"{audience_block}\n"
            f"【平台】视频号 / 抖音 / 小红书\n\n"
            f"1. 社媒文案（100~200 字，不用 Markdown）：\n"
            f"   {caption_struct}\n"
            f"   语气：自然口语，不端着，带情绪感染力\n"
            f"   {caption_examples}\n"
            f"   ⚠️ 铁律：文案最后一行必须有 3~5 个 # 开头的话题标签\n\n"
            ...
            f"CAPTION: 文案内容\n"
            f"TITLE: 短标题")
        # 容错 parser：兼容 "TITLE:" "TITLE：" "**TITLE:**" "- TITLE:" 等 LLM 变体
        for line in raw.strip().splitlines():
            s = line.lstrip("- *#•·>").strip()
            # 归一化中英文冒号
            low = s.upper()
            if low.startswith("CAPTION:") or low.startswith("CAPTION："):
                caption = s.split(":", 1)[-1].split("：", 1)[-1].strip() or caption
                ...
                t = t.strip()
                if t:
                    short_title = t[:16]  # 只有清洗后非空才覆盖默认值
        log(f"社媒文案解析完成：title_len={len(short_title)}, caption_len={len(caption)}")
    except Exception as e:
        log(f"社媒文案生成失败: {e}")
    return caption, short_title
...
def _generate_cover_image(topic: str, short_title: str, script: list[dict]) -> str | None:
    ...
    if tone == "轻松":
        ...
        persona = "你是 BBC、The Economist、The New Yorker、WIRED 等国际新闻/科普杂志的封面视觉指导，熟悉新闻封面、信息图封面、人物深度报道封面的视觉语言。"
        audience_goal = (
            "【受众】视频号/抖音/小红书泛知识用户\n"
            "【目标】3 秒传达信息密度与专业感，产生'想看完'的求知冲动\n"
            "【标杆参考】BBC 新闻封面、Economist 封面、WIRED 科技封面 —— 清洁构图 + 冷峻色调 + 核心概念视觉化\n"
        )
        title_style = (
            f"【中文标题渲染要求（Nano Banana 2 直出）】\n"
            f"• 渲染这串精准中文：「{short_title}」（一字不差，字形完整）\n"
            f"• 字体：modern bold editorial sans-serif Chinese heiti，干净理性\n"
            f"• 位置：bottom 28% 左对齐或居中\n"
            f"• 样式：dark navy or black text on clean off-white or pale cream bar（BBC / Economist 风）\n"
            f"• 画面顶部渲染一条贯穿整幅的红色色带（BBC 红条标志性元素），高度约画面的 3-4%\n"
            f"• 大小：标题占画面宽度的 75-85%\n"
            f"• 绝对不要把标题拆字/错字/漏字，每个中文字必须清晰完整\n"
        )
        design_rules = (
            "【设计原则 · 杂志封面级】\n"
            "1. 主色：冷峻 2~3 色（deep blue / charcoal / off-white / muted gold / signal red 点缀），信息图风\n"
            "2. 主体：核心概念视觉化（数字 / 图表 / 剪影 / 地球仪 / 书本 / 箭头图 / 单一人物冷峻肖像），占画面 40-55%\n"
            "3. 光线：clean editorial lighting / studio overhead / 均衡曝光\n"
            "4. 情绪钩子：专业 / 深度 / 洞察 / 克制，不煽情\n"
            "5. 风格：现代编辑设计 / infographic / minimal editorial / 新闻摄影；禁卡通或老照片\n"
            "6. 辅助元素：细线分割 / 几何色块 / 数字标记\n"
        )

    producer_block = f"【制片人准则（请重点参考其 STYLE_KEY / PALETTE / THUMBNAIL_ANCHOR 字段）】\n{producer_brief}\n\n" if producer_brief else ""

    # 让 LLM 按 tone 出视觉 prompt（Nano Banana 2 能渲染中文，主路径让它直出带标题的封面）
    try:
        ...
        raw = chat(
            ...
            (
                ...
                f"• 必须把中文标题「{short_title}」作为精确字符串嵌在英文 prompt 里，指令模型渲染；例如：render the exact Chinese title \"{short_title}\" in large bold heiti on a yellow bar at bottom center\n"
                f"• 其他部分描述主体、光线、构图、风格、色板\n"
                f"• 结尾加 'high detail, sharp typography, crisp readable Chinese characters, no watermark'（不要写 no text！因为就是要有文字）\n"
...
def step10_deliver(final_path: str, topic: str, script: list[dict]):
    caption, short_title = _generate_caption(topic, script)
    tg(f"📝 社媒文案 + 短标题已生成\n\n📤 正在发送...")
