...
| **B33 step6 错峰提交 12s 优化** | ★★ P2 | 30min | pending | step6 storyboard fallback / motion_bridge_refs / storyboard_grid 错峰提交默认 stagger=12s (env ADR_MOTION_BRIDGE_REFS_SUBMIT_STAGGER / ADR_GPT_IMAGE2_STORYBOARD_GRID_SUBMIT_STAGGER) · 观沧海 144959 run 7 个 fallback batch 错峰 6 次 × 12s = 72s 浪费 · 实际 WeryAI submit rate limit 现有动态退避已 cover · **修方向**: stagger 默认降到 5-6s, 触发 429 时 IMAGE_RATE_LIMIT_BACKOFF (15s) 自动退避 · 收益: step6 fallback / motion_bridge / grid 阶段总省 1-2min/run |
