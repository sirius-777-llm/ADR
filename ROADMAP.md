...
| **B31 HADS/VADS 也用 meta_grid 召唤** | ★★★ P1 | 1-1.5h | pending | 大哥洞察: HADS/VADS 当前用 storyboard_grid 切 12 panel 单图主路径, 跨 scene 角色漂移 (观沧海曹操、黄仁勋演讲等单 visual subject 题材). ADSD 已有 meta_grid 召唤机制 (整 grid 喂 WERYDANCE + 召唤式 prompt), HADS/VADS 应复用但去掉"强制说话"约束改成 "visual continuity reference" · **修方向 (Codex 推荐融合, 先 A 灰度后 B 结构化)**: A (30min-1h) 扩 meta_grid_active 加 HADS/VADS 条件 (env ADR_HADS_USE_META_GRID 灰度) + step1 ADS 提取 visual_subject (LLM 一次调用) + step65 motion_audio_dub 用 meta_grid_call prompt variant (改 "说话" → "visual continuity"). B (1-1.5d) 新增 visual_anchor 中间层解耦 speaker 链路 · 风险: visual_subject 抽错整片人物错乱; LLM cost 增 1 调用/run · 待下波 ship 决策 |
