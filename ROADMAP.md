...
| **B25 _PODCAST_TO_VOICE_ASSET_MAP pingshu 映射纠正** | ★★ P2 | 15min | pending | L811 mapping `pingshu-c7c18f5a` (古今先生评书风格) → `external_liang_wendao_yt_001` (梁文道, 文化评论非评书) 不一致 · 库里有真评书音色: external_yi_zhongtian (易中天评书腔 + 历史讲述) / external_shan_tianfang (单田芳旧时代评书) / external_tian_lianyuan (田连元沉稳评书) · 推荐: pingshu → external_yi_zhongtian_yt_001 (易中天题材兼容性最广) · 影响所有评书风格题材的 voice_asset clone |
...
| **B37 storyboard_grid 还原题材具体场景** | ★★ P2 | 1-2h | pending | 大哥实测反馈"画面没有尝试还原曹操写诗环境, 编剧功力不到位" · root: meta_grid_曹操 跨片复用 (B31), 含变装/动作 12 panel 但跟"碣石观沧海"题材具体场景 (山顶望海/秋风/星汉/写诗) 关联弱 · 7 panel motion 都用 meta_grid 复用, WERYDANCE 实际渲染时跟题材脱节 · **修方向**: (A) storyboard_grid 12 panel 应该体现 7 句具体场景 (东临碣石/水何澹澹/树木丛生/秋风萧瑟/日月之行/星汉灿烂/幸甚至哉) 而不只是 meta_grid 复用 · (B) jiangwen LLM 出 visual prompt 时强制每句对应一个具象场景词 (碣石/山岛/树木/秋风/日月/星汉/写诗), B14 anchor 兜底升级版 |
