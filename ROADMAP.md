...
| **B34 grid_multiref_motion 升 ADS 主路径** | ★★★ P1 | 1h | pending (CCP Stage 1+2 过, 待 confirm ship) | 大哥洞察: ADS 真正高质路径是 grid_multiref_motion (整 storyboard_grid 一次喂 WERYDANCE 出长视频 + ffmpeg 切回 N seg, 跨 scene 内部时序+风格统一), 不是 per_scene motion (每 scene 独立 5-15s, 角色/风格漂移). 现状: `STORYBOARD_GRID_MULTIREF_SEGMENTS` 默认 OFF (需 `--use-grid-multiref-segments` 显式启用), grid_multiref 仅 sidecar QA, 不切主时间线. ADS_STORYBOARD_FLOW_DEFAULT 设了 MOTION+MAIN 但没设 SEGMENTS · **修方向 (Codex 融合 B 先 A 后)**: B (30-45min) 加 `--grid-multiref-primary` CLI flag / env ADR_GRID_MULTIREF_PRIMARY 灰度入口, 启用时 SEGMENTS=True 触发 `_apply_grid_multiref_segments` 作主时间线, QA log 强化 pass_ratio/fallback_reason; A (B34.1, 5min) 数据达标后默认 ON · 跟 B31 协同: 主路径 grid_multiref 自带 visual continuity 不用 meta_grid, fallback per_scene 才用 meta_grid · 风险: MIN_PASS_RATIO 不达 fallback, B28+B29 已修 fallback 健壮性 |
