...
| **B34.1 grid_multiref 拆 group + duration 拉满** | ★★★ P1 | 30min | pending | 大哥实测 B34 反馈: combined 10s 内 7 panel = 1.4s/panel **切换太快没读出曹操气势** · root: L11141 `_grid_multiref_duration` cap 10s + L957 `ADR_STORYBOARD_GRID_MULTIREF_GROUP` setdefault 12 让 7 panel 全 1 组 · **修方向**: group_size 默认改 4 (env 仍可 override), duration cap 提到 15s (WERYDANCE 硬上限). 7 panel 拆 4+3 两组, 每组 15s, 总 ~25s ≥ audio 16.7s 无需 pad. 解决缺点 2+3. |
| **B34.2 grid_multiref prompt 改 panel-motion 一对一** | ★★ P2 | 30min | pending | 大哥实测反馈: **人物镜头语言高度相似, 背景换了也没大感觉** · root: `_grid_multiref_prompt` (L11152) 把 7 条 motion_prompts concat 整段, WERYDANCE 不知每 panel 对应哪条 motion · **修方向**: prompt 改成 "panel 1 visual=..., motion=...; panel 2 visual=..., motion=..." 明确 panel-motion 一对一映射 + 提示 WERYDANCE 顺序遍历 panel 用每段 motion. 解决缺点 4. |
| **B34.3 grid_multiref 切片对齐 audio turn boundary** | ★★ P2 | 1h | pending | 当前 `_apply_grid_multiref_segments` 用比例切分 (unsafe split), panel 切换 boundary 不跟 audio turn 对齐 · **修方向**: 用 motion_prompts 中每 turn 的 target_dur 累加得到精确切片点 (不是 total/N 比例切), ffmpeg trim 按 cumulative dur 切回 N seg. 配合 B34.1 拆 group 后, 每 group N panel 对应 N audio turn, 切片精度可控. |
