...
| **B29 step65 motion fast-fail** | ★★ P2 | 30min | pending | 防 audio_dub 头 2-3 个 turn 连续 timeout 后整 step65 烧 25min · ThreadPoolExecutor 内统计 timeout count, 触发 abort step65 整阶段 raise + 提示用户 (WERYDANCE 上游异常重试) · 跟 B28 配合: B28 单 turn 走 fallback, B29 多 turn 集体失败时整阶段熔断 · env ADR_MOTION_FAST_FAIL=1 默认开 |
