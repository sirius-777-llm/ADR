...
| **B50 speaker IP voice_asset 优先级** | 30-45min | ★★★ | planned | 143918 实测大哥反馈"没有曹操的角色音色". root: 曹操.json IP voice_asset_id=external_mushenji_lingjing_001 (慕神纪灵境道人, 显式角色音色) 完全没用上 · 现状: step1 LLM 按 topic 重选 podcast voice (评书风格→易中天) 忽略 IP 自带 voice_asset · **修方向**: step1 / _PODCAST_TO_VOICE_ASSET_MAP 解析前, 先查 script[0].speaker 对应 IP 文件 voice_asset_id 字段, 存在则覆盖 LLM 选择. IP > podcast mapping > gender fallback 三级优先. 影响所有 IP 已有 voice_asset 的 speaker (曹操/科比/乔丹等) |
