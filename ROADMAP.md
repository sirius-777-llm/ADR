...
| **B43 Flash Lite Tier 路由重分散** | 10min | ★★★ | planned | Flash Lite 429 quota 修复 D 方案 (PM 推荐先 ship): `_LLM_TIER["review"]` Flash Lite → GEMINI_25_FLASH (review 任务量小 quality 不降), `_LLM_TIER["data"]` Flash Lite → CLAUDE_HAIKU_4_5 (PR-4 已白名单 3 处验证). Flash Lite 退出 ADR 主路径只留直接 chat() 调用 4 点 (L3563 spielberg / L3358 scene_count / L3310 基调 / L9290 BGM). 收益: 单 run Flash Lite 调用 6-10 → 2-4 次, quota 分散到 3 model · 实测背景: 观沧海 101437 run 封面底部注脚 L16289 触发 429 quota exhausted |
