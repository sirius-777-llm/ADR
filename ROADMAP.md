...
| ~~**B24 HADS 单旁白默认用 master TTS audio**~~ | ~~★★★ P1~~ | ~~30min~~ | ✅ shipped 2026-05-27 (CCP Stage 4) | 「观沧海」HADS 14 turn 全单旁白罗翔, 但旁白音色跨 turn 不一致 · root cause: step9 L15352-15370 partial_ready check, 12/14 audio_dub 成功 + 2 fallback → coverage 0.857 ≥ 0.85 误触发 use_embedded_dialogue_audio=True → step9 用 WERYDANCE 段内 clone audio (上游模型 stochastic 跨 turn 漂移) 不用 master_voice.mp3 (TTS deterministic) · **修**: HADS/VADS WITH_MOTION 路径默认跳过 embedded audio_dub, 强制 master TTS audio; env `ADR_HADS_USE_EMBEDDED_AUDIO=1` 显式 opt-in 保留旧路径 · ADSD 路径不变 (L15340 audio_dub_experiment 仍走 embedded) · 嘴型 misalign 风险: HADS 旁白通常空镜远景, 不显 |
