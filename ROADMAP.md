...
| **PR-A.1 step66 _batch_groups 检测 hidden bug** | ★★ P2 | 1-2h | pending | 2026-05-27 罗永浩 HADSD 实测: 第一次 LLM 给 turn 5+6 罗永浩 (multi_turn_groups=1 QA 写入), 但 step66 `_batch_groups` 仍空, 12 个独立 lip_sync_raw, 0 个 merged_a · 加 debug log 后第二次跑 LLM 把 turn 完全打散 (multi_turn=0) 无法复现 · `_lip_sync_one_group` 函数本身经 tools/test_pr_a_merged_a.py 4/4 PASSED 验证功能正确 · **修方向**: 用 mock script 在 step66 主流程构造 multi_turn_group → 跑 `step66_adsd_lip_sync` 端到端 → 看 _batch_groups 是否真的填了 + tg "🚀 PR-A merged_a 启用" 是否发出 · 或者直接给已有 turn_timeline 加 "--force-group=N,M" CLI flag 强制注入测试 group |
