...
| **B10 ADS 单旁白多音色** | ★★★ P1 | 1h | planned (2026-05-26 议定) | **现象**: ADS (HADS/VADS) 单旁白模式跨 scene 音色不一致 · **Root cause**: step65 motion audio_dub 调 almighty 时 `_select_voice_asset_reference(scene, mode="motion")` per-scene 选 (line 12473)；scene 无 voice_asset_id → 走 gender fallback → 不同 scene 不同 asset；即使 asset 一致，每 scene 独立 almighty task → WeryAI voice cloning 端波动 · **方案**: 阶段 1 (30min) step1 ADS 模式末尾锁定 1 个 voice_asset 写入所有 scene["voice_asset_id"] + 新增 `_PODCAST_TO_VOICE_ASSET_MAP` 映射表 · 阶段 2 (30min) `_select_voice_asset_reference` 加 ADS 模式权威优先级 (scene voice_asset_id 存在则禁 gender override) · 阶段 3 (留 PR-A merged_a) 同 task 一致性 |
