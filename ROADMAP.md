...
| **B44 chat() 加 429 智能退避** | 2-3h | ★★★ | planned | Flash Lite 429 修复 A 方案 (配 B43 兜底): L1681-1706 chat() retry loop 加 `_is_llm_rate_limited_error(e)` 检查 (含 "Resource has been exhausted"/"quota"/status=429), 429 时退避 30s/60s/120s (vs 通用 3s/6s 线性), 跟 IMAGE_RATE_LIMIT_BACKOFF 对齐, 共用 helper 或独立 LLM_RATE_LIMIT_BACKOFF 常量. B43 ship 后 B44 是 Flash 2.5 / Haiku 也 429 时的兜底退避 |
...
| **B26 audio_dub fail rate 优化** | ★★★ P1 | 1-2h | pending | 大哥反馈"旁白音色不一致" 真 root cause: HADS 14 turn 中 audio_dub fail (rate limit / timeout) → fallback image-to-video silent → 整片有空白段 + 12 段成功 clone 跨 turn stochastic 漂移 · 修方向: (1) WERYDANCE submit 失败后 retry 2-3 次 (现 spike 数据看 retry +30% 成功) (2) PR-A.2 rate limit burst 优化协同 (3) silent fallback 段插 master TTS audio fill 空白 让连贯 · 评估完整路径后实施 |
