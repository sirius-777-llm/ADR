...
| **B13 silent_b 不该计入 ADSD speaker_count** | ★★ P1 | 30min | pending | 2026-05-27「南京的学区房」HADSD run: 真人 speaker 只 4 个 (旁白/刘平/李薇/王方) 但 delivery_qa.speaker_count=5 (silent_b 占位 turn 的 `(silent)` 被算第 5 个) · 触发 "too many ADSD speakers: 5" + "ensemble speaker_count mismatch" delivery fail · 副作用: adsd_voice_assign 还给 (silent) 分了 external_luo_xiang voice — 双重浪费 · **修方向**: speaker_count 统计前过滤 `(silent)` / `_is_silent_b()` / turn_type==silent_b; adsd_voice_assign 同步跳过 silent turn |
| **B14 storyboard prompt 缺台词主题锚定** | ★★★ P1 | 1-2h | pending | 2026-05-27「南京的学区房」HADSD run: text_visual_alignment_qa 12 中 10 失败, reason=`dialogue_core_terms_not_reflected_in_visual_prompt` · 台词 core terms (南京/学区/孩子/名校) 没体现, prompt 都是通用 "editorial magazine documentary photography..." 模板 · match_count=0 / required=2 · **修方向**: step1 storyboard prompt 增加 anchor 段: 每个 scene 从 dialogue text 抽 2-3 个 core entity (人物/地点/物品/情绪), 显式注入 visual prompt 必须包含 · 现代/抽象题材尤其重要 (古装题材 culture guard 已 cover 大头) |
| **B15 asr_qa.json 缺失 warning** | ★ P3 | 30min | pending | 2026-05-27 同 run delivery_qa warning: `ASR text match QA missing: asr_qa.json` · HADSD 用 dialogue_master.wav 但没跑 ASR 反查 turn text 匹配 · **修方向**: step2_master_voice / _asr_verify_dialogue_audio 落地后写 asr_qa.json; 或 delivery_qa 把 HADSD 的 ASR 校验降级为 optional 不发 warning |
| **B16 meta_grid 漏入 audio_dub postprocess** | ★★★ P1 | 30min | pending | 2026-05-27「南京的学区房」HADSD run t=85.0-85.6s 成片出现 4×3 meta_grid 满帧 (seg_10 末尾 ~0.6s) · **root cause**: `_postprocess_audio_dub_segment` (L12470) 在 raw < target_dur 时走 panel_pad 路径 (L12515), `panel_path = scene["img_path"]` 指向 `meta_grid_李薇.png`, Ken Burns 把 12 宫格当 still 缩放推满屏 · **历史教训**: B2 (5/21) 在 step7 防御层 (L13937) 加了 "img_path 是 meta_grid_* 不当 still" guard, 但 step66 audio_dub postprocess 路径没复用 · **修方向 (最小)**: L12515 `use_panel_pad` 加 `not basename(panel_path).startswith("meta_grid_")` 条件, 是 grid 时跳过, 走 L12555 tpad clone 末帧兜底 · 影响所有 ADSD/audio_dub + raw 比 audio 短的 turn |
