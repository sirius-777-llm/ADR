...
| **B32 step2 text-to-audio 走错 voice 浪费 credit** | ★★ P2 | 15min | pending | step2_master_voice L4721 use_text_to_audio 条件让 podcast-format picked_id (含 "-", LLM 输出常态) 仍走 text-to-audio 主路径 · 但 text-to-audio voice catalog 跟 Podcast 完全两套 (text-to-audio 用 voice_id 整数, Podcast 用 picked_id 字符串) · text-to-audio 永远拿不到 LLM 选的 voice, 用 hardcode voice_id=78 (女声) 跑 → 每次浪费 1 credit + 必走 Podcast fallback · **修方向**: 改 use_text_to_audio 条件为 `picked_id.isdigit() or not "-" in picked_id`, podcast-format picked_id 直接走 Podcast 主路径 |
